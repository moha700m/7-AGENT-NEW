...
CREATE POLICY "Admins can manage all leads" ON leads
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('super_admin', 'admin', 'staff')
    )
  );

-- ============================================
-- RLS Policies - Agents (Public Read)
-- ============================================
...
SELECT 
  ...
  COUNT(CASE WHEN status = 'جديد' THEN 1 END) as new_leads,
  COUNT(CASE WHEN status = 'تم التواصل' THEN 1 END) as contacted,
  COUNT(CASE WHEN status = 'تم البيع' THEN 1 END) as sold
FROM leads;
